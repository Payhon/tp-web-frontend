...
import { NButton, NCard, NDataTable, NForm, NFormItem, NInput, NModal, NSelect, NSpace, NTag, useDialog, useMessage } from 'naive-ui';
import type { DataTableColumns } from 'naive-ui';
import { useTable } from '@/hooks/common/table';
import { getDealerList, getEndUserDevices, getEndUserList, forceUnbindEndUserDevice as forceUnbindEndUserDeviceApi } from '@/service/api/bms';
...
const deviceColumns = computed<DataTableColumns<DeviceItem>>(() => [
  ...
  { key: 'is_owner', title: '主用户', width: 90, render: r => <NTag type={r.is_owner ? 'success' : 'default'}>{r.is_owner ? '是' : '否'}</NTag> },
...
const { data, loading, columns, pagination, getData, updateSearchParams } = useTable<EndUserItem, typeof getEndUserList>({
  ...
  <div class="flex-vertical-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <NCard title="终端用户" :bordered="false" size="small" class="card-wrapper">
      <NForm inline :model="searchForm" label-placement="left" label-width="auto" class="mb-4 flex flex-wrap gap-4 items-end">
        <NFormItem label="经销商" path="dealer_id">
          <NSelect v-model:value="searchForm.dealer_id" :options="dealerOptions" clearable style="width: 220px" placeholder="全部" />
        </NFormItem>
      ...
      <NDataTable :columns="columns" :data="data" :loading="loading" :pagination="pagination" :row-key="row => row.user_id" :scroll-x="960" />
    </NCard>
    ...
    <NModal v-model:show="showDevicesModal" preset="card" style="width: 860px" title="绑定设备">
      <NSpace vertical :size="12">
        ...
        <NDataTable :columns="deviceColumns" :data="devices" :loading="devicesLoading" :pagination="false" :row-key="row => row.binding_id">
