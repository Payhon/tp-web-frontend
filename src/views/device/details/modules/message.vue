...
const renderFormControl = (item: ExtensionInfo, index: number) => {
  ...
  switch (type) {
    case 'String':
      return <NInput v-model:value={additionInfo.value[index].value} placeholder={`默认值: ${default_value || ''}`} />
    case 'Number':
      return (
        <NInputNumber
    ...
    case 'Boolean':
      return <NSwitch v-model:value={additionInfo.value[index].value} checkedValue={true} uncheckedValue={false} />
    case 'Enum':
      return (
        <NSelect
          v-model:value={additionInfo.value[index].value}
              ...
              <div class="flex-1">
                <component
                  :is="
                    renderFormControl(
                      item,
                      additionInfo.findIndex(originalItem => originalItem.name === item.name)
                    )
                  "
                />
              </div>
