...
export const alertStatusDefinition: ComponentDefinition = {
  ...
  configComponent: AlertStatusSetting,
  defaultConfig: { type: 'alert-status', root: { transform: { rotate: 0, scale: 1 } }, customize: customConfig },
  config: { type: 'alert-status', root: { transform: { rotate: 0, scale: 1 } }, customize: customConfig },
  defaultLayout: { gridstack: { w: 3, h: 2, x: 0, y: 0, minW: 2, minH: 2, maxW: 6, maxH: 4 } },
  layout: {
  ...
  permission: '不限',
  tags: ['告警', '状态', '监控'],
  features: { realtime: true, dataBinding: true, configurable: true },
  dataSources: [
    {
      key: 'title',
      name: '标题',
